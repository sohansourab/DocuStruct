...
DEMO_USERNAME = "admin"
DEMO_PASSWORD = "docustruct"  # nosec B105
LOGO_SVG = """
...
if page == "📤 Upload & Process":
    ...
    for f in new_files:
        ...
        with st.container():
            ...
            with cols[1]:
                ...
                st.caption(
                    f"OCR status: **{result['ocr_status']}**{cache_note} · stored as document #{result['doc_id']}"
                )
                ...
                if already_seen:
                    st.info(
                        "This exact image was already processed this session — re-shown from OCR cache."
                    )

                st.caption(
                    "Full structured output, raw OCR text, and line items are available in Database Explorer."
                )
    ...
    if not rows:
        ...
        if not view.empty:
            ...
            if items:
                st.table(
                    pd.DataFrame(items)[["description", "quantity", "unit_price", "line_total"]]
                )
            else:
    ...
    if not rows:
        ...
        with c1:
            st.plotly_chart(
                px.histogram(df, x="overall_confidence", nbins=10, title="Confidence distribution"),
                use_container_width=True,
            )
        with c2:
            spend = df.groupby("vendor", dropna=True)["total"].sum().reset_index()
            st.plotly_chart(
                px.bar(spend, x="vendor", y="total", title="Total spend by vendor"),
                use_container_width=True,
            )
        ...
        if st.session_state.recent_results:
            ...
            st.caption(
                f"Ground-truth cases scored: {scorecard['known_cases']}"
                if scorecard["known_cases"]
                else "Upload one of the bundled sample receipts to compute ground-truth field accuracy."
            )
        else:
